...
var Select = React.createClass({
	...
	propTypes: {
		...
		onFocusChange: React.PropTypes.func,       // onFocusChange handler: function (value) {}
		...
		alwaysOpen: React.PropTypes.bool,          // whether or not to keep the list always visible
		selectOnly: React.PropTypes.bool,          // whether or not to update the value field when a selection is made
	},
	...
	getStateFromValue (value, options, placeholder) {
		...
		if (!this.props.multi && values.length) {
			...
			focusedOption = null//this.getFirstFocusableOption(filteredOptions);
			valueForState = values.map((v) => { return v[this.props.valueKey]; }).join(this.props.delimiter);
		}

    this.fireFocusChangeEvent(focusedOption)
		return {
			value: valueForState,
			values: values,
			inputValue: '',
	...
	setValue (value, focusAfterUpdate) {
		...
		newState.isOpen = this.props.alwaysOpen;
		this.fireChangeEvent(newState);
		this.setState(newState);
	},
  
  setFocus (focusedOption) {
	...
	fireFocusChangeEvent (focusedOption) {
		if (focusedOption !== this.state.focusedOption && this.props.onFocusChange) {
			this.props.onFocusChange(focusedOption);
		}
	},
	...
	handleInputChange (event) {
      ...
      var focusedOption = this._getNewFocusedOption(filteredOptions)
      this.fireFocusChangeEvent(focusedOption)
			this.setState({
				isOpen: true,
				inputValue: event.target.value,
	...
	focusAdjacentOption (dir) {
		...
		if (!this.state.isOpen) {
      var focusedOption = this.state.focusedOption || ops[dir === 'next' ? 0 : ops.length - 1]
      this.fireFocusChangeEvent(focusedOption)
			this.setState({
				isOpen: true,
				inputValue: '',
	...
	unfocusOption (op) {
      ...
      this.fireFocusChangeEvent(null)
			this.setState({
				focusedOption: null
			});
		}
    ...
    var activeValue = this.props.activeValue
		...
		var ops = Object.keys(options).map(function(key) {
			...
			var isActive = activeValue === op[this.props.valueKey];
	...
	render () {
		...
		if (!this.state.inputValue && (!this.props.multi || !value.length)) {
			...
			if (this.props.valueRenderer && !!this.state.values.length) {
				...
				var singleValueComponent = React.createElement(this.props.singleValueComponent, {
					key: 'placeholder',
					value: val,
					placeholder: this.props.selectOnly ? this.props.placeholder : this.state.placeholder
				});
		...
		// indicator arrow
		var arrow = this.props.alwaysOpen ? null : (
